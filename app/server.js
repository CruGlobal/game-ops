...
import { reconcileWeeklyStreaks } from './services/streakService.js';
...
// Reconcile weekly streaks daily at midnight. A streak counts the workdays contributed
// in the current week, so this is what carries everyone across the Monday boundary and
// corrects any value that drifted from the contribution rows.
cron.schedule('0 0 * * *', async () => {
    logger.info('Running daily task to reconcile weekly streaks');
    try {
        if (!(await shouldRunCron('streakCheck'))) return;
        const result = await reconcileWeeklyStreaks();
        logger.info('Streak reconciliation done', { checked: result.checked, updated: result.updated });
    } catch (error) {
        logger.error('Error reconciling streaks', { error: error.message });
    }
});
