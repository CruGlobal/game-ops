...
describe('ContributorService', () => {
  ...
  describe('getTopContributors', () => {
    it('should return top contributors sorted by PR count', async () => {
      ...
      expect(result[0].prCount).toBe(15);
      expect(result[1].username).toBe('user1');
      expect(result[1].prCount).toBe(10);
    });
  ...
  describe('getTopReviewers', () => {
    it('should return top reviewers sorted by review count', async () => {
      ...
      expect(result[0].reviewCount).toBe(30);
      expect(result[1].username).toBe('reviewer1');
      expect(result[1].reviewCount).toBe(20);
    });
  });
  ...
  describe('awardBillsAndVonettes', () => {
    it('should award Bill for 10+ PRs', async () => {
      ...
      const updatedContributor = await Contributor.findOne({ username: 'productive' });
      expect(updatedContributor.totalBillsAwarded).toBe(1);
    ...
    it('should award Bill for 10+ reviews', async () => {
      ...
      const updatedContributor = await Contributor.findOne({ username: 'reviewer' });
    ...
    it('should award Vonette for 500+ PRs', async () => {
      ...
      const updatedContributor = await Contributor.findOne({ username: 'superstar' });
      expect(updatedContributor.totalBillsAwarded).toBe(5);
    ...
    it('should award incremental Bills based on total contributions', async () => {
      ...
      const updatedContributor = await Contributor.findOne({ username: 'consistent' });
      expect(updatedContributor.totalBillsAwarded).toBe(2);
    });
